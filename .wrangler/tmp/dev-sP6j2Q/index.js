...
// node_modules/hono-rate-limiter/dist/index.js
var getResetSeconds = /* @__PURE__ */ __name((resetTime, windowMs) => {
  let resetSeconds;
  if (resetTime) {
...
var setDraft6Headers = /* @__PURE__ */ __name((context, info, windowMs) => {
...
var setDraft7Headers = /* @__PURE__ */ __name((context, info, windowMs) => {
  if (context.finalized) return;
  const windowSeconds = Math.ceil(windowMs / 1e3);
...
var setRetryAfterHeader = /* @__PURE__ */ __name((context, info, windowMs) => {
  if (context.finalized) return;
  const resetSeconds = getResetSeconds(info.resetTime, windowMs);
...
var isValidStore = /* @__PURE__ */ __name((value) => !!value?.increment, "isValidStore");
...
function honoRateLimiter(config) {
  const {
    ...
    keyGenerator,
    skip = /* @__PURE__ */ __name(() => false, "skip"),
    requestWasSuccessful = /* @__PURE__ */ __name((c) => c.res.status < 400, "requestWasSuccessful"),
    handler = /* @__PURE__ */ __name(async (c, _, options2) => {
  ...
  return async (c, next) => {
    ...
    const shouldSkipRequest = /* @__PURE__ */ __name(async () => {
...
__name(honoRateLimiter, "honoRateLimiter");
function cloudflareRateLimiter(config) {
  const {
    ...
    keyGenerator,
    skip = /* @__PURE__ */ __name(() => false, "skip"),
    handler = /* @__PURE__ */ __name(async (c, _, options) => {
...
__name(cloudflareRateLimiter, "cloudflareRateLimiter");
var scripts = {
  increment: `
  ...
  add(method, path, handler) {
    ...
    const parts = (path.match(/\/?(:\w+(?:{(?:(?:{[\d,]+})|[^}])+})?)|\/?[^\/\?]+/g) || []).map(
      (part) => {
        ...
        return match2 ? `/(?<${match2[1]}>${match2[2] || "[^/]+"})` : part === "/*" ? "/[^/]+" : part.replace(/[.\\+*[^\]$()]/g, "\\$&");
      }
    );
    try {
      this.#routes.push([
  ...
  match(method, path) {
    ...
    for (let i = 0, len = this.#routes.length; i < len; i++) {
      ...
      if (routeMethod === method || routeMethod === METHOD_NAME_ALL) {
        const match2 = pattern.exec(path);
        if (match2) {
          handlers.push([handler, match2.groups || emptyParams2]);
        }
      }
    }
    return [handlers];
...
// node_modules/@everapi/freecurrencyapi-js/index.js
var Freecurrencyapi = class {
  static {
    __name(this, "Freecurrencyapi");
  }
  ...
  call(endpoint, params = {}) {
    ...
    return fetch(`${this.baseUrl}${endpoint}?${paramString}`, { headers: this.headers }).then((response) => response.json()).then((data) => {
      return data;
    });
  }
  status() {
...
var freecurrencyapi_js_default = Freecurrencyapi;

// src/api-client.ts
var apiClient = null;
var cachedApiKey = null;
var getApiClient = /* @__PURE__ */ __name((apiKey) => {
  if (apiClient && cachedApiKey === apiKey) {
    return apiClient;
  }
  apiClient = new freecurrencyapi_js_default(apiKey);
  cachedApiKey = apiKey;
  return apiClient;
}, "getApiClient");
...
app.get("/", async (c) => {
  try {
    const apiClient2 = getApiClient(c.env.API_KEY);
    const resp = await apiClient2.currencies();
    return c.json(resp);
  } catch (e) {
    console.error(e);
...
app2.get("/", async (c) => {
  ...
  try {
    const apiClient2 = getApiClient(c.env.API_KEY);
    const resp = await apiClient2.latest({ base_currency: base, currencies });
    return c.json(resp);
  } catch (e) {
    console.error(e);
    const message = e instanceof Error ? e.message : "Internal Server Error";
...
var latest_default = app2;

// src/index.ts
var api = new Hono2();
var app3 = new Hono2();
api.route("/currencies", currencies_default);
api.route("/latest", latest_default);
app3.use(
  rateLimiter({
    binding: /* @__PURE__ */ __name((c) => c.env.CC_PROXY_RATE_LIMITER, "binding"),
    keyGenerator: /* @__PURE__ */ __name((c) => c.req.header("cf-connecting-ip") ?? "", "keyGenerator")
  })
);
app3.route("api/", api);
...
// .wrangler/tmp/bundle-GivqSh/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-GivqSh/middleware-loader.entry.ts
